                ...
                <div className="flex gap-2">
                  {["Tudo", "Não lidas", "Grupos"].map((filter) => (
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-wider transition-all ${activeFilter === filter ? "bg-primary text-white" : "bg-slate-100 text-slate-400"
                        }`}
                    >
                      {filter}
                    </button>
              ...
              <div className="flex-1 overflow-y-auto px-2 py-4 custom-scrollbar">
                {filteredChats.map((chat) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all mb-1 ${selectedChat?.id === chat.id ? "bg-white shadow-lg shadow-slate-200/50 scale-[1.02] z-10" : "hover:bg-white"
                      }`}
                  >
